...
const Home: NextPage = ({ images }: { images: ImageProps[] }) => {
  ...
  useEffect(() => {
    ...
    if (lastViewedPhoto && !photoId) {lastViewedPhotoRegf.current.scrollIntoView({ block: 'center' })
  ...
  return (
    <>
        ...
        <div className="columns-1 gap-4 sm:columns-2 xl:columns-3 2xl:columns-4">
          ...
          {images.map(({ id, public_id, format, blurDataUrl }) => (
            <Link
              ...
              href={`/?photoId=${id}`}
              as={`/p/${id}`}
              ref={id === Number(lastViewedPhoto) ? lastViewedPhotoRegf : null}
              shallow
              className="after:content group relative mb-5 block w-full cursor-zoom-in after:pointer-events-none after:absolute after:inset-0 after:rounded-lg after:shadow-highlight"
            >
              <Image
                alt="Sankalp Sharma photography"
      ...
      <Avatar className='p-6 text-center sm:p-12'>
